...
// GET public mentors (no authentication required)
export async function GET(request: NextRequest) {
  try {
    ...
    const mentors = await Mentor.find(filter)
      .select('name designation experience expertise skills avatar bio socialLinks teachingExperience rating students courses achievements')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();
